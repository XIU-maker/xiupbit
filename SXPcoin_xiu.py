...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=3600):
            target_price = get_target_price("KRW-SXP", 0.36)
            target_high = get_target_price("KRW-SXP", 0.53)
            current_price = get_current_price("KRW-SXP")
            sxp = get_balance("SXP")
            if target_price < current_price and sxp == 0:
                krw = get_balance("KRW")
                if krw > 5000:
                    upbit.buy_market_order("KRW-SXP", krw*0.9995)
            elif target_high < current_price and sxp > 6:
