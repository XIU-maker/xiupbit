...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=3600):
            target_price = get_target_price("KRW-FCT2", 0.36)
            target_high = get_target_price("KRW-FCT2", 0.53)
            current_price = get_current_price("KRW-FCT2")
            fct2 = get_balance("FCT2")
            if target_price < current_price and fct2 == 0:
                krw = get_balance("KRW")
                if krw > 5000:
                    upbit.buy_market_order("KRW-FCT2", krw*0.9995)
            elif target_high < current_price and fct2 > 60:
