...
while True:
    try:
        ...
        start_time = get_start_time("KRW-BTG") #9:00
        end_time = start_time + datetime.timedelta(days=1) #9:00 + 1일

        # 9:00 < 현재 < #8:00:00
        if start_time < now < end_time - datetime.timedelta(seconds=3600):
            target_price = get_target_price("KRW-BTG", 0.5)
            target_row1 = get_target_row("KRW-BTG", 0.82)
            target_row2 = get_target_row("KRW-BTG", 0.72)
            current_price = get_current_price("KRW-BTG")
            if target_price < current_price:
                krw = get_balance("KRW")
                if krw > 8000000:
                    upbit.buy_market_order("KRW-BTG", krw*0.69)
                ...
                if target_row2 > current_price:
                    ...
                    upbit.buy_market_order("KRW-BTG", krw*0.5)
