...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=3600):
            target_price = get_target_price("KRW-EOS", 0.12)
            target_high = get_target_price("KRW-EOS", 0.36)
            current_price = get_current_price("KRW-EOS")
            eos = get_balance("EOS")
            if target_price < current_price and eos == 0:
                krw = get_balance("KRW")
                if krw > 5000:
                    upbit.buy_market_order("KRW-EOS", krw*0.9995)
            elif target_high < current_price and eos > 3.8:
