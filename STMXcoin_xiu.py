...
while True:
    try:
        ...
        if start_time < now < end_time:    
            target_price = get_target_price("KRW-STMX", 0.010)
            target_high = get_target_price("KRW-STMX", 0.105)
            current_price = get_current_price("KRW-STMX")
            stmx = get_balance("STMX")
            if target_price < current_price and stmx == 0:
                krw = get_balance("KRW")
                if krw > 5000:
                    upbit.buy_market_order("KRW-STMX", krw*0.9995)
            elif target_high < current_price and stmx > 380:
