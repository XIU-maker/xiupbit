...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=3600):
            target_price = get_target_price("KRW-IOTA", 0.36)
            target_high = get_target_price("KRW-IOTA", 0.53)
            current_price = get_current_price("KRW-IOTA")
            iota = get_balance("IOTA")
            if target_price < current_price and iota == 0:
                krw = get_balance("KRW")
                if krw > 5000:
                    upbit.buy_market_order("KRW-IOTA", krw*0.9995)
            elif target_high < current_price and iota > 12:
