...
while True:
    try:
        ...
        start_time = get_start_time("KRW-BSV") #9:00
        end_time = start_time + datetime.timedelta(days=1) #9:00 + 1일

        # 9:00 < 현재 < #8:00:00
        if start_time < now < end_time - datetime.timedelta(seconds=3600):
            target_price = get_target_price("KRW-BSV", 0.5)
            target_row1 = get_target_row("KRW-BSV", 0.82)
            target_row2 = get_target_row("KRW-BSV", 0.72)
            current_price = get_current_price("KRW-BSV")
            if target_price < current_price:
                krw = get_balance("KRW")
                if krw > 8000000:
                    upbit.buy_market_order("KRW-BSV", krw*0.69)
            ...
            btc = get_balance("BSV")
            if btc > 0.00009:
                upbit.sell_market_order("KRW-BSV", btc*0.9995)
        time.sleep(5)
