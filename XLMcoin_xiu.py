...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=3600):
            target_price = get_target_price("KRW-XLM", 1.00)
            target_high = get_target_price("KRW-XLM", 1.75)
            current_price = get_current_price("KRW-XLM")
            xlm = get_balance("XLM")
            if target_price < current_price and xlm == 0:
                krw = get_balance("KRW")
                if krw > 5000:
                    upbit.buy_market_order("KRW-XLM", krw*0.9995)
            elif target_high < current_price and xlm > 35:
